...
#' 
#' Note: this adapts original code by Bertagnolli et al. I simply fix
#' a bug whereby lengths had to be specified, otherwise it would result
#' in an error. The algorithm itself is not modified.
#'
...
GCE_weighted <- function(g, directed = FALSE, normalised = TRUE, weights = NULL,
  ...
  if (!use_lengths) {
    ...
    g_all_shortest_paths <- intsegration::rcpp_floyd_flow(x)
    rm(x)
  } else {
    ...
    g_all_shortest_paths <- intsegration::rcpp_floyd_flow_length(x, l)
    rm(x)
    rm(l)
  }
  # split outputs
