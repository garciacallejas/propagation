...
# obtain species presences across the grid in different scenarios
# 1 - removal of invasive plants
# 2 - TODO 

# this script is adapted from 00_get_subset_communicability
# it will calculate the composition for each grid cell independently, 
# or if needed, accounting for the immediate neighborhood
...
# -------------------------------------------------------------------------
# parameters for the species scenarios

# percentage of cells to modify
cells.modified <- 0.1
...
# maximum distance for the neighborhood (in km)
max.dist <- 30

sp.int.orig <- read.csv2("results/plant_bird_interactions_clean.csv")
sp.disp <- read.csv2("results/species_dispersal_rates.csv")
...
# read observations and cell grid
...
# -------------------------------------------------------------------------
# add species status (native/exotic) to cell observations
sp.status <- sp.traits %>% select(species,guild,status) %>% unique()
sp.cells.2 <- left_join(sp.cells,sp.status)
...
sp.removal.list <- sort(unique(sp.traits$species[sp.traits$guild == "plants" & 
                                                   sp.traits$status == "Exotic"]))

# random selection of cells, for the first test
my.cell.ids <- sample(cell.id,length(cell.id)*cells.modified,replace = F)

untreated.cells <- subset(sp.cells.2,!(cell_id %in% my.cell.ids))
treated.cells <- subset(sp.cells.2,cell_id %in% my.cell.ids)
treated.cells <- subset(treated.cells,!(species %in% sp.removal.list))

invasive.removal.scenario <- arrange(bind_rows(untreated.cells,treated.cells),cell_id)
...
# -------------------------------------------------------------------------
# for each cell, 
# populate this subset of the landscape,
# and run the species scenarios N times.

# Return the relative frequencies of these N runs, in the form
# of an adjacency matrix for each cell.

# tests
# i.cell <- 453
# cell.id <- 1:10

s1.list <- foreach(i.cell = 1:length(cell.id), 
                     # .combine=comb.fun, 
                     .packages = 'tidyverse') %dopar% {
                       ...
                       my.sp.list <- subset(sp.cells.2, cell_id == cell.id[i.cell])
                       
                       # i will decide whether to have different scripts for each scenario,
                       # or do it all in a single one
                       
                       my.cell.birds <- my.sp.list$species[my.sp.list$guild == "birds"]
                       my.cell.plants <- my.sp.list$species[my.sp.list$guild == "plants"]
                       ...
                       df.names <- expand.grid(all.sp,sublandscape.ids)
                       ...
                       sub.dist.list <- reshape2::melt(sub.distances)
                       names(sub.dist.list) <- c("cell1","cell2","distance")
