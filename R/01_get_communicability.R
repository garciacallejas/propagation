...
# }
# 
# save(binary.comm.matrix,file = "results/binary_communicability_matrix.Rdata")
# save(weighted.comm.matrix,file = "results/weighted_communicability_matrix.Rdata")
# 
# rm(block.matrix)
# gc(verbose = F)
...
if(is.binary){
  ...
  # TODO consider other options to make it faster, e.g. renaming matrix
  # dimensions as sp_grid (1_1, etc), and do an automatic conversion to dataframe
  
  for(i.pos in 1:nrow(df1)){
    
    # my.sp1 <- which(all.sp == df1$sp1[i.pos])
    # my.grid.id.sp1 <- df1$grid.id.sp1[i.pos]
    # my.sp2 <- which(all.sp == df1$sp2[i.pos])
    # my.grid.id.sp2 <- df1$grid.id.sp2[i.pos]
    
    my.matrix.row <- num.sp * (df1$grid.id.sp1[i.pos] - 1) + df1$sp1[i.pos]
    my.matrix.col <- num.sp * (df1$grid.id.sp2[i.pos] - 1) + df1$sp2[i.pos]
    
    df1$binary.communicability[i.pos] <- binary.comm.matrix[my.matrix.row,my.matrix.col]
  ...
  df1$diag <- ifelse(df1$sp1 == df1$sp2,TRUE,FALSE)
  ...
  for(i.pos in 1:nrow(df1)){
    ...
    my.matrix.row <- num.sp * (my.grid.id.sp1 - 1) + my.sp1
    my.matrix.col <- num.sp * (my.grid.id.sp2 - 1) + my.sp2
...
write_csv2(df1,"results/communicability_pairwise.csv",row.names = FALSE)
