...
# -------------------------------------------------------------------------
# read/calculate data with correlated degree and dispersal?
correlated.deg.disp <- T

# -------------------------------------------------------------------------

if(correlated.deg.disp){
  read.sim.landscape.path <- "/home/david/Work/datasets/NZ/results/sim_landscape_matrices_correlated_deg_disp/"
  save.comm.path <- "/home/david/Work/datasets/NZ/results/communicability/sim_correlated"
  save.net.comm.path <- "/home/david/Work/datasets/NZ/results/communicability/sim_correlated/network_level/"
}else{
...
# -------------------------------------------------------------------------
if(correlated.deg.disp){
  netcom.df <- list.files("/home/david/Work/datasets/NZ/results/communicability/sim_correlated/network_level/",full.names = T) %>% map_dfr(read.csv2)
  # netcom.df$X <- NULL
  write.csv2(netcom.df,"results/sim_correlated_network_level_communicability.csv",row.names = F)
}else{
