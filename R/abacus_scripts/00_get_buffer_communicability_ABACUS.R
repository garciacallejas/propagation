...
library(tidyverse)
library(sf)
# source("communicability_network.R")
# source("communicability.R")
...
# path to data in abacus
abacus.path <- "/share/data/people/dga82/propagation_data/"
abacus.scripts.path <- "/home/dga82/propagation_scripts/"
abacus.results.path <- "/share/data/people/dga82/propagation_results/"

# abacus.path <- "./results/"
# abacus.results.path <- "./results/"
# abacus.scripts.path <- "R/"
...
max.dist <- c(0,1,2,3,4,5) * grid.size

sp.int.orig <- read.csv2(paste(abacus.path,"plant_bird_interactions_clean.csv",sep=""))
sp.disp <- read.csv2(paste(abacus.path,"species_dispersal_rates_",grid.size,"km.csv",sep=""))

# what to assign to birds without HWI measure? 
hwi <- read.csv2(paste(abacus.path,"bird_hand_wing_index.csv",sep=""))
...
if(observation.source == "observed_records"){
  sp.cells <- read.csv2(paste(abacus.path,"sp_observations_long_",grid.size,"km.csv",
                              sep=""))
}else if(observation.source == "occurrence_prob"){
  sp.cells <- read.csv2(paste(abacus.path,"model_occurrences_",grid.size,"km.csv",
                              sep=""))
...
NZ.grid <- st_read(paste(abacus.path,"NZ_grid_",grid.size,"km.shp",sep=""))
...
# tests
cell.id <- 671 # 671 is in the middle of the north island
i.cell <- 1
# null.list <- foreach(i.cell = 1:length(cell.id), 
#         # .combine=comb.fun, 
#         .packages = 'tidyverse') %dopar% {
  
  source(paste(abacus.scripts.path,"communicability_network.R",sep=""))
  source(paste(abacus.scripts.path,"communicability.R",sep=""))
...
# for(i.cell in 1:length(cell.id)){
  ...
  for(i.dist in 1:length(max.dist)){
    sublandscape.ids <- which(my.cell.distances <= max.dist[i.dist])
    ...
    }# if max.dist[i.dist]>0, i.e. if surrounding cells are considered
    ...
    cell.df <- df1 %>% 
      ...
      mutate(buffer.distance = max.dist[i.dist])
    
    write.csv2(cell.df,paste(abacus.results.path,"buffer_",max.dist[i.dist],"_focal_",i.cell,"_",grid.size,"km.csv",sep=""))
    
  }# for each distance
  
  # cell.df <- bind_rows(cell.df.list)
  
    # -------------------------------------------------------------------------

    # write.csv2(cell.df,paste(abacus.results.path,"increasing_buffer_focal_",i.cell,"_",grid.size,"km.csv",sep=""))
